...
def config_generation(model_type, path, task, data_format, na_hierarchy_tag):
    ...
    sweep_config = {
        ...
        "parameters": {
            ...
            "max_text_length": {"values": [32, 64, 128]},
...
def main(config):
    ...
    if config.na_hierarchy:
        na_hierarchy_tag = "--na_hierarchy"
    else:
        na_hierarchy_tag = "--flat"
    sweep_config = config_generation(model_type=config.model_type, path=config.data_path, task=task, data_format=data_format, na_hierarchy_tag=na_hierarchy_tag)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--na_hierarchy", action='store_true', default=False)
